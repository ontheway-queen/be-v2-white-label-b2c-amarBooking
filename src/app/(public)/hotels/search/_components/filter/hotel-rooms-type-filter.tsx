...
export const HotelRoomsType = memo(() => {
  const dispatch = useAppDispatch();
  const { defaultOption, selectedOption } = useAppSelector(useHotelFilter);

  const state = defaultOption?.roomTypes as string[];
  const selected = selectedOption?.roomTypes;
  ...
  const handleChange = (value?: string) => {
    ...
    dispatch(setHotelFilters({ roomTypes: updatedValue }));
  };

  return (
    <HotelAccordion value={'roomTypes'} label={'Room Type'} className='truncate'>
...
HotelRoomsType.displayName = 'HotelRoomsType';
